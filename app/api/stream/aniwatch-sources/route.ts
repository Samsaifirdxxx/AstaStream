...
// Initialize the Zoro (Aniwatch) provider
...
export async function GET(request: Request) {
  try {
    ...
    const searchResults = await zoro.search(animeTitle);
    ...
    // Get episode info
    const animeInfo = await zoro.fetchAnimeInfo(anime.id);
    ...
    const sources = await zoro.fetchEpisodeSources(episodeData.id);
    ...
    console.error("Error fetching Aniwatch sources:", error);
    return NextResponse.json(
